...
async function fetchSearchedPlayerName(searchTerm) {
    ...
    async function getResponse() {
        var theresponse = await axios.get("https://www.balldontlie.io/api/v1/players?search=" + searchTerm);
...
function searchArea(searchArray, playerNum) {
    ...
    for (i=0;i<searchArray.length; i++) {
        ...
        searchResults.textContent = searchArray[i].first_name + " " + searchArray[i].last_name;
    ...
    clearButton.addEventListener("click",function(){
        searchBox.innerHTML = "";
        if (playerNum === 1) {player1SearchButton.style.display = "block"}
        if (playerNum === 2) {player2SearchButton.style.display = "block"}
    })
...
player1SearchButton.addEventListener("click", async function () {
    ...
    searchArea(await fetchSearchedPlayerName(playerName1Input.value.toString()), 1);
})
player2SearchButton.addEventListener("click", async function () {
    searchArea(await fetchSearchedPlayerName(playerName2Input.value.toString()), 2);
})
